...
{
	if(pos < 0) return LIST_BOF;
	if(pos >= list_size(list)) return LIST_EOF;

	int counter = 0;
	for(ListNode current_node = list_first(list); current_node != LIST_EOF;
		current_node = list_next(list, current_node))
	{
		if(counter == pos)
		{
			Pointer list_node_val = list_node_value(list, current_node);
			return list_node_val;
		}
...
{
	ListNode last_node = list->dummy;
	for(ListNode current_node = list_first(list); current_node != LIST_EOF;
		current_node = list_next(list, current_node))
	{
		if(current_node == node)
		{
			list_remove_next(list, last_node);
			break;
		}
